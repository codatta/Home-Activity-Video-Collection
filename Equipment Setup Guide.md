# 📱 Equipment Setup Guide

## Low-Cost Solutions for Body Camera Setup

---

## Option 1: Chest Mount (Recommended ⭐)

### 🛒 Buy Ready-Made

**Search Keywords**:
- "chest mount phone holder"
- "GoPro chest strap smartphone"
- "body camera mount"


**Popular Items**:
1. Universal chest strap + phone clip 
2. GoPro-compatible chest mount + phone adapter 

---

### 🔧 DIY Alternative

**Materials Needed**:
- Backpack chest strap (existing)
- Car phone holder or desk stand
- Zip ties / strong tape

**Steps**:
1. Attach phone holder to center of chest strap
2. Secure with zip ties
3. Adjust angle to point forward-downward

---

## Option 2: Head Mount

### 🛒 Buy Ready-Made

**Search Keywords**:
- "head strap phone mount"
- "POV camera headband"
- "GoPro head mount smartphone"


**Tip**: Choose adjustable size for comfort

---

### 🔧 DIY Version

**Materials**:
- Sports headband / elastic band
- Phone clamp
- Velcro / zip ties

**Steps**:
1. Fix phone mount at center of headband
2. Balance weight to prevent tilting
3. Test stability (shake head gently)

---

## Option 3: Cap Modification (Creative)

**Materials**:
- Baseball cap / cap with brim
- Small phone clip
- Strong glue / sewing kit

**Steps**:
1. Mount phone clip above brim center
2. Ensure firm attachment
3. Adjust shooting angle

---

## 📐 Camera Angle Guide

### Correct Angle

    [Camera]
      ↓ 15-30°
    👐👐
   [Object]

 
### Checklist
- [ ] Both hands visible in frame
- [ ] Object being operated is clear
- [ ] No excessive shaking
- [ ] Comfortable for 10+ minutes
---
## 💡 Practical Tips
1. **Test First**: Record 1 minute to check angle
2. **Good Lighting**: Daytime or turn on lights
3. **Secure Mount**: Avoid loosening during recording
4. **Power Bank**: Prepare for long recordings
5. **Storage**: Ensure enough phone space
---
## 🛍️ Regional Shopping Links
**Shopee** (Singapore/Malaysia/Thailand/Philippines):
Search: "chest mount phone"
Search: "head strap GoPro phone"

**Lazada** (SEA region):
Search: "body camera mount smartphone"
Search: "POV recording equipment"

**Expected Prices**:
- Chest mount: $5-12 USD
- Head mount: $8-15 USD
- Cap mount: $3-8 USD (excluding cap)
---
## ⚠️ Safety Notes
1. Secure phone firmly to prevent drops
2. Adjust tightness for comfort
3. Check battery before recording
4. Disable notifications to avoid interruption
---
## 🌏 Southeast Asia Specific Tips
**Thailand**: 
- JD Central, Shopee TH have good selection
- MBK Center (Bangkok) for physical stores
**Vietnam**:
- Shopee VN, Tiki.vn
- Phone accessories shops in District 1 (HCMC)
**Philippines**:
- Shopee PH, Lazada PH
- Greenhills Mall (Manila) for accessories
**Indonesia**:
- Tokopedia, Shopee ID
- ITC malls in Jakarta
**Malaysia**:
- Shopee MY, Lazada MY
- Low Yat Plaza (KL) for tech accessories
---
*Last Updated: 2026-04-18*
